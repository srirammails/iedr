-- Name: depositTopups
-- Parameters: nic = nichandle to search DepositHist
--             	fromDate = date to search from
--             	toDate = date to search to
--		the to and from dates are inclusive.
-- example : call DepositTopups('IDL2-IEDR', '2011-02-01 09:38:48', '2011-02-18 09:26:27');


DELIMITER //

DROP PROCEDURE IF EXISTS `depositTopups`; 

CREATE PROCEDURE DepositTopups(nic varchar(20), fromDate datetime, toDate datetime )
BEGIN
	set @a=1; set @b=1; 
	select dh1.rownum1, dh2.rownum2,  dh1.Nic_Handle, dh1.Op_Dt, dh1.Open_Bal, dh1.Close_Bal, dh2.Open_Bal, dh2.Close_Bal, ABS(dh1.Close_Bal - dh2.Close_Bal) AS topup, dh2.Order_ID   
	from (
	...
	where dh2.rownum2 = dh1.rownum1 + 1  
		...
		AND dh2.Close_Bal > dh1.Close_Bal
