...
function getNewRegCount()
        {

$query="select count(*) as x from Domain where A_Number=600 and D_Reg_Dt >=DATE_FORMAT(NOW(), '%Y-%m-01') and D_Reg_DT < CURDATE() + Interval 1 day";
//Connect to the DB
    $connection = @ mysql_connect($GLOBALS [hostName],
                                $GLOBALS [username],
                                $GLOBALS [password])
