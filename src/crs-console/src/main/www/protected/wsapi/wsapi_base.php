...
function decode($a) {
    if (isset($a)) {
        return htmlspecialchars_decode($a, ENT_QUOTES);
    }
    return null;
}
 ...
 * generated by CRS-WS build script scripts/crs-ws-scripts/run_build.sh
 */
...
function parseXmlDate($ds)
	{
      $d = null;
      if($ds and is_string($ds))  {
         $ds = substr($ds, 0, 10);
         $d = date('Y-m-d',strtotime($ds));
      }
      return $d;
	}

function parseXmlDateWithDefault($ds) {
    $d = parseXmlDate($ds);
    return $d == '1970-01-01' ? 'N/A' : $d;
}
 ...
 * @version   Release: @package_version@
 * @link      https://statistics.iedr.ie/
 * @see       wsapi/wsdl_from_svc/get_wsdl.sh
 */
class crs_wsapi_service_base
     ...
     */
	static $crs_soap_options = array('soap_version'=>SOAP_1_1,'exceptions'=>true,'trace'=>1,'cache_wsdl'=>WSDL_CACHE_MEMORY, 'compression'=>SOAP_COMPRESSION_ACCEPT);
	...
	static function getMessageFromException($exception,$expected_list)
		{
		if (strcmp($exception->faultcode, 'HTTP') == 0) {
            $msg = 'Error while connecting to the backoffice. ' . $exception->faultstring;
            Yii::log($msg);
			return $msg;
		}
