...
class Nichandle_Details
     ...
     * @access public
     */
	public $nonPhone_fieldList = array('nicHandleId','address','companyName','country','county','email','name','billCInd','vatNo');
   ...
   public function rules() {
      $rules = array(
         array('companyName, email, address, country, county, phones', 'required'),
         array('phones,faxes', 'validatePhoneNumbersArray'),
         array('name, companyName, address, email, country, county', 'length', 'max' => 254),
         array('email', 'EmailAddrValidator'),
         array('vatNo', 'VAT_Number_Validator'),
         array('country', 'validateCountry'),
         array('county', 'validateCounty'),
         array('returningData', 'safe'),
      );
      if ($this->nicHandleId == Yii::app()->user->name) {
         array_push($rules, array('name', 'required'));
      }
      return $rules;
   }
   ...
   function validateCounty($attrib, $params){
      $counties = getCountyOptions($this->country);
		...
		if(is_array($this->$attribarray))
			...
			foreach($this->$attribarray as $n => $p)
				...
				if(!(preg_match(PhoneNumberValidator::$phoneNumberRegex, $p)===1))
					...
					$msg = ''.encode($p).' is not a valid '. $field .' Number';
