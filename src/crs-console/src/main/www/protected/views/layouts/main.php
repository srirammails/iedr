...
<div class="container" id="page">
	<table class="header">
		<tr>
			<td rowspan="2" class="no_padding logo"><img src="images/ielogo.jpg"></td>
			<td class="no_padding top">
				<?php
				   if(isset(Yii::app()->user->authenticatedUser) && (Utility::getPermission() > 0) ) {
				?>
				<iframe scrolling="NO" height="26" width="700px" src="<?php echo $this->createUrl('site/summaryframe'); ?>" class="user_panel"></iframe>
				<?php
				   }
				?>
			</td>
			...
			<td class="bottom no_padding">
				<?php 
					if(isset(Yii::app()->user->authenticatedUser)) {
						$this->widget('zii.widgets.CMenu', array('htmlOptions'=>array('class'=>'sf-menu'),'items' => getMenu())); 
					} else {
						$this->widget('zii.widgets.CMenu', array('htmlOptions'=>array('class'=>'sf-menu'),'items' => getMenuForQuest())); 
					} 
				?>
			</td>
		</tr>
