...
$this->endWidget();
echo '</div>';
echo "<p>There were " . $model->totalrows . " domains transferred away from your account over the last " .  $model->days . " days or " . round($model->totalrows/date('t'),2) . " per day</p>";
...
$w = $this->widget('application.widgets.jqGrid.jqGridWidget', array(
        ...
        'thisfile'              => $this->createUrl('account_view_transfers_away/menu',array('model'=>$model_serialised)),
        'datasource'            => $this->createUrl('account_view_transfers_away/getgriddata_away',array('model'=>$model_serialised)),
        'model'                 => $model,
        ));
?>
