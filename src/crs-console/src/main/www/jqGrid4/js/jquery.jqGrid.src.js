...
$.extend($.jgrid,{
	...
	from : function(source,initalQuery){
		...
		this.startsWith=function(f,v){
			...
			var dates = val.toString().split(" ", 2);    
			if(_useProperties){
				if(dates.length == 2) {
				    var param = '('+self._getStr('jQuery.jgrid.getAccessor(this,\''+f+'\')')+'.substr(0,'+(dates[0].length)+') >= '+self._getStr('"'+self._toStr(dates[0])+'"') + ') && (';
				    param += self._getStr('jQuery.jgrid.getAccessor(this,\''+f+'\')')+'.substr(0,'+(dates[1].length)+') <= '+self._getStr('"'+self._toStr(dates[1])+'"') + ')';
				    self._append(param);
				} else {
				...
				if(dates.length == 2) {
				    self._append(self._getStr('this')+'.substr(0,'+length+') == '+self._getStr('"'+self._toStr(dates[0])+'"'));
				    self._append(self._getStr('this')+'.substr(0,'+length+') == '+self._getStr('"'+self._toStr(dates[1])+'"'));
				} else {
...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
        ...
        setPager = function (pgid, tp){
            ...
            str = "<td dir='"+dir+"' >No. of records per page:</td>";
				str += "<td dir='"+dir+"' >";
		...
		return this.each(function(){
			...
			var triggerToolbar = function() {
				...
				$.each($t.p.colModel,function(i,n){
					...
					switch (this.stype) {
						...
						case 'date':
							so  = (this.searchoptions && this.searchoptions.sopt) ? this.searchoptions.sopt[0] : p.defaultSearch;
							vFrom = $("#gs_"+$.jgrid.jqID(this.name)+"_from", $t.grid.hDiv).val();
							vTo = $("#gs_"+$.jgrid.jqID(this.name)+"_to", $t.grid.hDiv).val();
							if(vTo || vFrom) {
			...
			$.each($t.p.colModel,function(i,n){
				...
				if(this.search){
					...
					case 'date':
						var df = soptions.defaultValue ? soptions.defaultValue: "";
 						$(thd).append("<input type='text' style='width:60px;padding:0px;' name='"+(cm.index || cm.name)+"_from' id='gs_"+cm.name+"_from' value='"+df+"'/>");
						if(soptions.attr) {
						...
						$(thd).append("<input type='text' style='width:60px;padding:0px;' name='"+(cm.index || cm.name)+"_to' id='gs_"+cm.name+"_to' value='"+df+"'/>");
						if(soptions.attr) {
						    $("input",thd).attr(soptions.attr);
						}
						
						if(soptions.dataInit !== undefined) { 
						    soptions.dataInit($("input",thd)[1]); 
						}
						
						if(soptions.dataEvents !== undefined) {
						    bindEvents($("input",thd)[1], soptions.dataEvents); 
						}
