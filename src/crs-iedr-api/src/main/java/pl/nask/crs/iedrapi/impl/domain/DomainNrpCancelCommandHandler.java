 ...
 */
public class DomainNrpCancelCommandHandler extends AbstractDomainCommandHandler<MNameType> {
	...
	public ResponseType handle(AuthData auth, MNameType command, ValidationCallback callback) throws AccessDeniedException, IedrApiException {
		...
		try {
			...
			getDomainAppService().removeFromVoluntaryNRP(auth.getUser(), names.toArray(new String[names.size()]));
			return ResponseTypeFactory.success();
		} catch (DomainIllegalStateException e) {
			throw new ObjectStatusProhibitsOperationException(ReasonCode.DOMAIN_VNRP_REMOVE_NOT_POSSIBLE, new Value("name", DOMAIN_NAMESPACE, e.getDomain().getName()), e);
		} catch (Exception e) {
